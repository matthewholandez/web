...
Single Next.js 16 (App Router) app; **pnpm** is the package manager (`pnpm-lock.yaml`). Standard scripts are in `package.json`: `pnpm dev` (dev server on port 3000), `pnpm build`, `pnpm start`, `pnpm lint`. No database, backend, env vars, or external services are needed — the `/time` countdown is fully client-side. Styling is plain CSS (no Tailwind).

- Dev/test target: `http://localhost:3000` (routes: `/`, `/now`, `/projects`, `/time`). Run the dev server with `pnpm dev`.
- About copy: `content/about.md`. Now updates: `content/now/YYYY-MM-DD.md`.
